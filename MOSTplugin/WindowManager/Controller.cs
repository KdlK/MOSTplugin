...
namespace MOSTplugin.WindowManager
{
    public abstract class ModelData
    {
        ...
        protected List<Model> GetModelData()
        {
            ...
            List<Element> DoorsCollector = (from door in (new FilteredElementCollector(Data.Doc).WhereElementIsNotElementType().OfCategory(Innercategory))
                                            ...
                                            where KSIparam != null
                                            where KSIparam.AsString().StartsWith("ACDA")
                                            select door).ToList();
    ...
    public static class AutoDimension
    {
        ...
        public static void CreateDimension(Model model)
        {
            ...
            //}
            PutDimension("imposts", new List<int>() { 0, 1 }, model, "bottom", 500);
        ...
        private static void PutDimension(string orientation,List<int> indexes, Model model, string side, double offset) {
            ...
            List<DetailLine> impostLines = new List<DetailLine>();
            ...
            if (orientation == "vertical" || orientation == "imposts") // vertical 
            {
                ...
                if (orientation == "imposts") { 
                    List<int> ints = new List<int>();
                    Double line_length = Data.Doc.GetElement(model.TypeId).LookupParameter("М_Створка_Высота").AsDouble();
                    if (Data.Doc.GetElement(model.TypeId).LookupParameter("М_СубКомп_Тип_2_Вкл").AsInteger() == 1) { 
                        PlanarFace leftImpostSide = sortedFaces[5];
                        PlanarFace RightImpostSide = sortedFaces[6];


                        XYZ startPoint = new XYZ(element_location.X+ (RightImpostSide.Origin.X + leftImpostSide.Origin.X)/2, element_location.Y+ leftImpostSide.Origin.Y, element_location.Z+ leftImpostSide.Origin.Z) ;
                        XYZ endPoint = new XYZ(element_location.X+ (RightImpostSide.Origin.X + leftImpostSide.Origin.X)/2, startPoint.Y  -line_length, element_location.Z + leftImpostSide.Origin.Z);


                        Line lineGeom = Line.CreateBound(startPoint, endPoint);
                        DetailLine detailLine = Data.Doc.Create.NewDetailCurve(model.LegendView, lineGeom) as DetailLine;
                        impostLines.Add(detailLine);

                    }
                    if (Data.Doc.GetElement(model.TypeId).LookupParameter("М_СубКомп_Тип_3_Вкл").AsInteger() == 1)
                    {
                        PlanarFace leftImpostSide = sortedFaces[9];
                        PlanarFace RightImpostSide = sortedFaces[10];
                    ...
                    if (Data.Doc.GetElement(model.TypeId).LookupParameter("М_СубКомп_Тип_4_Вкл").AsInteger() == 1)
                    {
                        PlanarFace leftImpostSide = sortedFaces[13];
                        PlanarFace RightImpostSide = sortedFaces[14];
            ...
            if (orientation == "imposts")
            {
                references.Add(sortedFaces.First().Reference);
                Options opt = new Options();
                opt.IncludeNonVisibleObjects = false;
                opt.ComputeReferences = true;
                foreach (DetailLine detailLine in impostLines)
                {
                    try
                    {
                        GeometryElement geomElem = detailLine.get_Geometry(opt);

                        Reference detailLineRef = null;

                        // Loop through the geometry objects within the detail line
                        foreach (GeometryObject geomObj in geomElem)
                        {
                            // We look for a Curve as detail lines should result in curve geometry
                            if (geomObj is Curve)
                            {
                                Curve curve = geomObj as Curve;

                                // This will give you the reference to this geometry curve
                                detailLineRef = curve.Reference;

                                references.Add(detailLineRef);
                                break;
                                 // Assuming we only care about the first curve found
                            }
                ...
                references.Add(sortedFaces.Last().Reference);
            }
            else
            {
